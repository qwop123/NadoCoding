...
int main(void)
{
	...
	while (1)
	{
		...
		if (checkAnimal[firstSelect_x][firstSelect_y] == 0 // 카드가 뒤집히지 않았는지
			&& checkAnimal[secondSelect_x][secondSelect_y] == 0 // 두 동물이 같은지)
			&& arrayAnimal[firstSelect_x][firstSelect_y] == arrayAnimal[secondSelect_x][secondSelect_y]) // 같은 동물은 경우
		{
...
void suffleAnimal()
{
	...
	for (int i = 0; i < 10; i++)  // 10마리의 동물
	{
		for (int j = 0; j < 2; j++ ) // 2장씩
		{
